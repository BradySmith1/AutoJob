...
function displayOverviewFields(field, billable) {
  ...
  if (field.name === "Name") {
    ...
    if (billable.inputs === undefined) {
      jsxElement = <h3 className="fourth">{field.name}:</h3>;
    } else {
      jsxElement = (
        <h3 className="fourth">
...
function Overview(props) {
  ...
  useEffect(() => {
    ...
    props.schema.form.forEach((stage, index) => {
      for (
        var i = 0;
        i < props.values.form[index][stage.canonicalName].length;
        i++
      ) {
        console.log(props.values.form[index][stage.canonicalName]);
        total =
          total +
          props.values.form[index][stage.canonicalName][i].price *
        ...
        {props.schema.form.map((stage, index) => (
          <div className="infoWrapper" key={index}>
            <div className="headerWrapper">
              ...
              <h3>
                Sub Total: $
                {getTotal(props.values.form[index][stage.canonicalName])}
              </h3>
            </div>
            <div className="divide"></div>
            {/**Map over the materials and display name, price, and quantity */}
            {props.values.form[index][stage.canonicalName].map(
              (billable, fieldIndex) => (
