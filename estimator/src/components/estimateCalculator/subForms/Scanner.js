...
function Scanner(props) {
  ...
  const [company, setCompany] = useState(
    props.scanBillable.billable.data.company !== undefined
      ? props.scanBillable.billable.data.company
      : "homedepot"
  );
  ...
  const formik = useFormik({
    ...
    initialValues: {
      zip:
        props.scanBillable.billable.data.zip !== undefined
          ? props.scanBillable.billable.data.zip
          : "",
    ...
    onSubmit: (values) => {
      ...
      axios
        ...
        .catch((error) => {
          ...
          setSearching("error");
          addMessage(
            "A network error has occured, cannot retrieve price.",
            5000
          );
        });
    },
  ...
  return (
    <div className="Back">
      <div className="formContainer" style={{ width: "350px" }}>
        <>
          <span className="compnayWrapper">
            ...
            <div className="CompanySelect">
              <button
                className="button seperate"
                style={
                  company === homeDepot ? selectedButton : deselectedButton
                }
                onClick={() => {
          ...
          <form id="scrapeForm" onSubmit={formik.handleSubmit}>
            ...
            <input className="btn" type="submit" id="submitButton" />
          </form>
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  newBillable.billable.data.price = Number(
                    foundItem.price.toFixed(2)
                  );
                  props.modifyLibrary(
                    props.scanBillable.index,
                    newBillable.billable.data
                  );

                  axios
                    .put(
                      "/api/library?_id=" + newBillable.billable.data._id.$oid,
                      newBillable.billable.data
                    )
                    .then((respone) => {
                      setSearching("tick");
                    ...
                    .catch((error) => {
                      addMessage(
                        "Network error, could not update price.",
                        5000
                      );
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    axios
                      .put(
                        "/api/library?_id=" +
                          newBillable.billable.data._id.$oid,
                        newBillable.billable.data
                      )
                      .then((respone) => {
                        console.log(respone);
        ...
        <button
          className="btn exit"
          style={{ position: "absolute", bottom: "-45px", left: "145px" }}
          onClick={() => {
