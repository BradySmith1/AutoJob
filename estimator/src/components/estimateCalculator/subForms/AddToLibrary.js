...
function AddToLibrary(props) {
  const formik = useFormik({
    ...
    validationSchema: Yup.object(generateFieldSchema(props.selectedLib.schema)),
    ...
    <div className="Back">
      <div className="formContainer">
        ...
        <div id="libraryForm">
          ...
          {formik.values.inputs !== undefined &&
            Object.keys(formik.values.inputs).map((key) => (
              <div className="boxAndLable" key={key}>
                <h3>{key}</h3>
                <input
                  className="inputBox customBox"
                  type={
                    typeof formik.values.inputs[key] == "number"
                      ? "number"
                      : "text"
                  }
                  id={`inputs[${key}]`}
