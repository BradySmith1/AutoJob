...
function schemaSwitch(unit) {
  ...
  switch (unit) {
    case "Text":
      ...
      break;
    case "Number":
      defaultValue = Yup.number().required("Required");
      break;
  }
  return defaultValue;
}
...
function generateFieldSchema(stage) {
  ...
  return {
    ...
    name: Yup.string().required("Required").max(20, "Maximum of 20 characters"),
    price: Yup.number().required("Required"),
    quantity: Yup.number().required("Required"),
    description: Yup.string().required("Required"),
    autoImport: Yup.string().required("Required"),
    autoUpdate: Yup.string().required("Required"),
  };
}
...
function generateYupSchema(schema) {
  ...
  schema.form.forEach((stage) => {
    blankSchema[stage.canonicalName] = Yup.array(
      Yup.object().shape(generateFieldSchema(stage))
    );
  });
...
function determineBillables(initialValues, data, schema) {
  ...
  schema.form.forEach((stage, index) => {
    ...
    localData.form.forEach((tempStage) => {
      ...
      if (
        dataStage !== undefined &&
        dataStage.hasOwnProperty(stage.canonicalName)
      ) {
...
function Estimator(props) {
  ...
  const validationSchema = useMemo(
    () => generateYupSchema(props.schema),
    [props.schema]
  );
  ...
  var initialValues = useMemo(
    () =>
      determineBillables(
        generateInitialValues(props.schema),
        props.data,
        props.schema
      ),
    []
  ...
  const postDraftData = (values, status) => {
    ...
    if (estimateData.hasOwnProperty("_id")) {
      ...
      axios
        .put("/api/estimate?_id=" + estimateData._id.$oid, estimateData, {
          timeout: 12000,
        })
        .then(() => {
      ...
      axios
        .post("/api/estimate", estimateData, { timeout: 3000 })
        .then((response) => {
          props.setData({
            ...props.data,
            _id: { $oid: response.data.insertedId.$oid },
          });
          //If the status is complete, delete from user estimates and
          //refresh the page
          axios
            .delete(`/api/user?_id=${estimateData.user._id.$oid}`, {
              timeout: 12000,
            })
            .then(() => {
              handleSubmission(status);
      ...
      <div className="divider">
        ...
        <div className="formNav">
          ...
          <button
            className="button"
            onClick={() => {
              setNavIndex(props.schema.form.length);
            }}
          >
            Overview
          </button>
          ...
          <Form>
            ...
            {navIndex === props.schema.form.length ? (
              ...
              <>
                ...
                {determineErrors(errors, props.schema) ? (
                  <div className="center invalid">
                    Input Errros Prevent Submission
                  </div>
                ) : null}
                {saved ? (
                ...
                {postError ? (
                  <div className="center">
                    <Message
                      message={
                        "A network error has occured, could not save estimate. Try again later."
                      }
                      setDisplay={setSaved}
                    />
                  </div>
